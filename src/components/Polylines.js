            ...
            var path = plan.points.map((point, j) => {
                ...
                if (j < plan.points.length-1) {
                    ...
                    ans = <line key={i.toString() + j.toString()}
                                x1={convertLng(parseFloat(curr[1]), this.props.width)}
                                y1={convertLat(parseFloat(curr[0]), this.props.height)}
                                x2={convertLng(parseFloat(next[1]), this.props.width)}
                                y2={convertLat(parseFloat(next[0]), this.props.height)}
